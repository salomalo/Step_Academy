...
void main()
{
	// створюємо ітератор вивідного потоку (cout) для виведення int-ів
//	ostream_iterator<int> itOutInt ( cout );			// такий ітератор виводить усі елементи підряд, зліплюючи їх
	...
	//Show( oVec2 , "oVec2" );
	cout <<"oVec2  : ";
	copy ( oVec2.begin() , oVec2.end() , itOutInt );	// копіюємо в потік виводу -- алгоритму copy байдуже КУДИ копіювати: він бачить лише ітератор
	cout <<endl;

	deque<int> oDec( oVec.rbegin() , oVec.rend() );
	//Show( oDec , " oDec" );
	cout <<" oDec  : ";
	copy ( oDec.begin() , oDec.end() , itOutInt );	// копіюємо в потік виводу -- алгоритму copy байдуже КУДИ копіювати: він бачить лише ітератор
	cout <<endl;
